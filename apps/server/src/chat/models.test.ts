...
describe("mock model streaming", () => {
	...
	test("preserves punctuation continuations across tool turns", async () => {
		expectNoProviderCalls = false;
		let callCount = 0;
		providerStream = async function* () {
			callCount += 1;
			...
			yield { type: "text_delta", delta: ": exposed the script." };
			yield {
				type: "done",
				reason: "stop",
				message: {
					...
					provider: "test",
					model: "model",
					content: [{ type: "text", text: ": exposed the script." }],
					usage: { input: 8, output: 2 },
					stopReason: "stop",
					timestamp: 2,
				},
