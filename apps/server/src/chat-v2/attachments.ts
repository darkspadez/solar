...
export function expandMessageAttachments(
	...
	if (attachments.length === 0)
		return { message: record.message, decisions: [] };
	const decisions: AttachmentDecision[] = [];
	if (record.message.role === "assistant")
		return {
			message: record.message,
			decisions: attachments.map((attachment) => ({
				messageId: record.id,
				attachmentId: attachment.id,
				decision: "unsupported_by_model",
			})),
		};
	const content =
	...
	for (const attachment of attachments) {
		if (
			...
			attachment.byteSize > capabilities.maxAttachmentBytes
		) {
			decisions.push({
				messageId: record.id,
				attachmentId: attachment.id,
				decision: "omitted_by_budget",
			});
			continue;
		}
		...
		decisions.push({
			...
			attachmentId: attachment.id,
			decision:
				resolved.type === "text" && resolved.summarized
					? "summarized"
					: "included",
		});
	}
	return {
		message: expanded
			? ({ ...record.message, content } as Message)
			: record.message,
		decisions,
	};
}
...
export class AttachmentService {
	...
	bind(
		...
		return this.repository.bindAttachment(
			userId,
			conversationId,
			messageId,
			attachmentId,
