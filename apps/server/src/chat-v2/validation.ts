	...
	constructor(reason: string, diagnostics: DiagnosticIds = {}) {
		const context = [
			diagnostics.conversationId &&
				`conversation=${diagnostics.conversationId}`,
...
function validateTextOrImageContent(
	...
	for (const [index, part] of content.entries()) {
		...
		if (block.type === "text")
			typedString(block.text, `${name}[${index}].text`, context);
...
function validateAssistantContent(
	...
	if (!Array.isArray(content))
		fail("assistant.content must be an array", context);
	for (const [index, part] of content.entries()) {
		...
		if (block.type === "text")
			...
			typedString(
				block.thinking,
				`assistant.content[${index}].thinking`,
				context,
			);
...
export function parseCanonicalMessage(
	...
	if (message.role === "assistant") {
		...
		if (
			!new Set(["stop", "length", "toolUse", "error", "aborted"]).has(
...
export function validateToolPairing(
	...
	const pending = new Map<
		string,
		{ name: string; context: MessageValidationContext }
	>();
	for (const [index, message] of messages.entries()) {
		...
		if (pending.size > 0 && message.role !== "toolResult") {
			...
			fail(
				`tool call ${id} must receive a result before the next ${message.role} message`,
				context,
			);
		}
		if (message.role === "assistant") {
		...
		if (call.name !== message.toolName)
			fail(
				`tool result ${message.toolCallId} name does not match ${call.name}`,
				context,
			);
...
export function repairDanglingToolCalls(
	...
	const pending = new Map<
		string,
		{ name: string; anchor: CanonicalMessageRecord }
	>();
	...
	for (const record of records) {
		...
		for (const part of (record.message as AssistantMessage).content) {
			if (part.type === "toolCall")
				pending.set(part.id, { name: part.name, anchor: record });
		}
	}
	flushPending();
	return repaired;
